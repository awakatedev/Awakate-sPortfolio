...
function Header() {
  ...
  if (winSize.width < 768) {
    return (
      <>
        <nav className="fixed z-20 flex fhd:h-16 hd:h-10 h-16 w-screen justify-between bg-blackBg/50 px-6 backdrop-blur-xl">
        ...
        {isModalOpen && (
          <div
            className={`fixed left-0 top-14 z-30 flex h-full w-full items-center justify-center bg-black/70 text-center ${
              isFading
                ? 'opacity-0 transition-opacity duration-300'
                : 'opacity-100'
            }`}
          >
            <ul className="mb-40 flex flex-col gap-8 fhd:text-4xl hd:text-base text-textGray">
              <li>
                <button
  ...
  return (
    <nav className="fixed z-20 flex h-16 w-screen justify-between bg-blackBg/40 px-6 backdrop-blur">
      <button type="button" onClick={() => router.push('/')}>
